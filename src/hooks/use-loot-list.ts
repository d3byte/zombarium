...
import {
  ...
  setStats,
  setWeapon,
} from 'hoc/withGameContext/actions';
...
export const useLootList = () => {
  ...
  const lootList = useMemo(
    () =>
      isInventory
        ? player.inventory
        : (openedObject?.loot?.filter((item) => !takenLootItemsIds.includes(item.id)) as ItemInterface[]),
    [isInventory, takenLootItemsIds, player.inventory, openedObject],
  );
  ...
  const applyConsumable = (item: ConsumableInterface) => {
    ...
    const debuffs = Array.from(
      new Set(
        [...player.debuffs, ...(item.debuffsInflicted || [])].filter(
          (debuff) => !item.debuffsRemoved?.includes(debuff.type),
        ),
      ),
    );
